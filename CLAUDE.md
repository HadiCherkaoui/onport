...
- **Docker detection**: Use `bollard` crate behind a `docker` feature flag. Connect to Docker socket, list containers, match PIDs via cgroup or port mapping. If socket unavailable, skip silently.
...
- Use `anyhow` for error handling in main, `thiserror` for library errors
